...
import { loadMealWindows, saveMealWindows, type MealWindow } from "@/features/groups/model/meal-windows";
import { AppShell } from "@/shared/components/app-shell";

export function GroupSettingsPage() {
  const [windows, setWindows] = useState(loadMealWindows);
  ...
  function updateWindow(mealType: MealWindow["mealType"], changes: Partial<MealWindow>) {
    setSaved(false);
    setWindows((current) => current.map((window) => window.mealType === mealType ? { ...window, ...changes } : window));
  }

  function handleSave() {
    if (hasInvalidRange) return;
    saveMealWindows(windows);
    setSaved(true);
  }
    ...
    <AppShell>
      <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        <div className="mt-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <div className="flex items-center gap-2"><p className="text-sm font-bold uppercase tracking-[0.16em] text-[var(--tomato)]">Configurações do grupo</p><span className="rounded-full bg-[var(--lemon)] px-2 py-1 text-[10px] font-extrabold uppercase">Proprietário</span></div>
            <h1 className="mt-2 font-serif text-4xl font-bold">Horários do bandejão</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-black/55">Defina quando cada refeição é servida. Todos os integrantes verão apenas horários dentro destes limites.</p>
          </div>
        </div>
        ...
        {saved && <p role="status" className="mt-5 rounded-2xl bg-[var(--sage)] px-4 py-3 text-sm font-semibold text-white">✓ Limites atualizados para todo o grupo neste protótipo.</p>}
        <button type="button" onClick={handleSave} disabled={hasInvalidRange} className="mt-6 w-full rounded-2xl bg-[var(--ink)] px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto">Salvar horários do grupo</button>
      </div>
    </AppShell>
  );
}
