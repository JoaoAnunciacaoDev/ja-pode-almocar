...
export function GroupMembersPanel({ group, currentUserId, onFeedback }: { group: GroupDetails; currentUserId: string; onFeedback: (message: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-[28px] border border-red-200 bg-white p-6">
      ...
      {isOwner ? <>
        <div className="mt-4"><h2 className="font-extrabold">Transferir propriedade</h2><p className="mt-1 text-sm leading-5 text-black/45">A pessoa escolhida passa a controlar integrantes, convites e configurações. Você permanece como integrante.</p>{members.length ? <div className="mt-3 flex flex-col gap-2 sm:flex-row"><select value={selectedNewOwnerId} onChange={(event) => setNewOwnerId(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-black/10 bg-white px-3 py-2.5 text-sm">{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select><button disabled={pending || !selectedNewOwnerId} type="button" onClick={() => { const member = members.find((item) => item.id === selectedNewOwnerId); if (member && window.confirm(`Transferir a propriedade para ${member.name}?`)) transferMutation.mutate(); }} className="rounded-xl border border-black/10 px-4 py-2.5 text-sm font-bold disabled:opacity-40">Transferir</button></div> : <p className="mt-3 text-sm text-black/40">Convide outro integrante antes de transferir.</p>}</div>
        <div className="mt-6 border-t border-red-100 pt-5"><h2 className="font-extrabold text-red-700">Excluir grupo</h2><p className="mt-1 text-sm leading-5 text-black/45">Apaga permanentemente integrantes, convites, rotinas e horários deste grupo.</p><button disabled={pending} type="button" onClick={() => { if (window.confirm(`Excluir permanentemente o grupo “${group.name}”? Esta ação não pode ser desfeita.`)) deleteMutation.mutate(); }} className="mt-3 rounded-xl bg-red-700 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">Excluir grupo</button></div>
      </> : <div className="mt-4"><h2 className="font-extrabold">Sair do grupo</h2><p className="mt-1 text-sm leading-5 text-black/45">Sua participação, seus horários e suas rotinas neste grupo serão apagados.</p><button disabled={pending} type="button" onClick={() => { if (window.confirm(`Sair do grupo “${group.name}”? Seus horários e rotinas neste grupo serão apagados.`)) leaveMutation.mutate(); }} className="mt-3 rounded-xl bg-red-700 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">Sair do grupo</button></div>}
