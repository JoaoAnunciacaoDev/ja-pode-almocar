...
export function ProfileSettingsPage() {
  ...
  return <AppShell><div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 lg:py-12">
    ...
    <div className="mt-8 space-y-5">
      ...
      <form onSubmit={(event) => { event.preventDefault(); const password = field(event, "password"); const confirmation = field(event, "confirmation"); if (password !== confirmation) { setError("As senhas não coincidem."); return; } void run("password", () => updatePassword(password), "Senha atualizada."); }} className="rounded-[24px] bg-white p-6"><h2 className="text-xl font-bold">Senha</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Nova senha<PasswordInput name="password" minLength={8} autoComplete="new-password" required /></label><label className="text-sm font-bold">Confirmar senha<PasswordInput name="confirmation" minLength={8} autoComplete="new-password" required /></label></div><button disabled={busy !== null} className="mt-4 rounded-2xl bg-[var(--ink)] px-5 py-3 text-sm font-bold text-white disabled:opacity-40">Alterar senha</button></form>
