...
export function ProfileSettingsPage() {
  ...
  return <AppShell><div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 lg:py-12">
    ...
    <div className="mt-8 space-y-5">
      ...
      <form onSubmit={(event) => { event.preventDefault(); const currentPassword = field(event, "currentPassword"); const password = field(event, "password"); const confirmation = field(event, "confirmation"); if (password !== confirmation) { setError("As senhas não coincidem."); return; } void run("password", () => updatePassword(currentPassword, password), "Senha atualizada."); }} className="rounded-[24px] bg-white p-6"><h2 className="text-xl font-bold">Senha</h2><p className="mt-2 text-sm text-black/50">Confirme sua senha atual antes de definir uma nova.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold sm:col-span-2">Senha atual<PasswordInput name="currentPassword" autoComplete="current-password" required /></label><label className="text-sm font-bold">Nova senha<PasswordInput name="password" minLength={8} autoComplete="new-password" required /></label><label className="text-sm font-bold">Confirmar senha<PasswordInput name="confirmation" minLength={8} autoComplete="new-password" required /></label></div><button disabled={busy !== null} className="mt-4 rounded-2xl bg-[var(--ink)] px-5 py-3 text-sm font-bold text-white disabled:opacity-40">Alterar senha</button></form>
      <form onSubmit={(event) => { event.preventDefault(); const currentPassword = field(event, "currentPassword"); void run("delete", async () => { await deleteOwnAccount(currentPassword); await navigate({ to: "/entrar", search: { invite: undefined } }); }, "Conta excluída."); }} className="rounded-[24px] border border-red-200 bg-red-50 p-6"><h2 className="text-xl font-bold text-red-800">Excluir conta</h2><p className="mt-2 text-sm text-red-700">A exclusão é permanente. Antes, transfira ou exclua os grupos dos quais você é proprietário.</p><label className="mt-4 block text-sm font-bold text-red-800">Senha atual<PasswordInput name="currentPassword" autoComplete="current-password" required /></label><label className="mt-4 block text-sm font-bold text-red-800">Digite EXCLUIR para confirmar<input value={deleteText} onChange={(event) => setDeleteText(event.target.value)} className={inputClassName} /></label><button disabled={deleteText !== "EXCLUIR" || busy !== null} className="mt-4 rounded-2xl bg-red-700 px-5 py-3 text-sm font-bold text-white disabled:opacity-40">Excluir minha conta</button></form>
    </div>
  </div></AppShell>;
}
